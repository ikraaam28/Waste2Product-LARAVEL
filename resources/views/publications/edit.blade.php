...
<div class="container-fluid publication py-5 my-5">
    <div class="container py-5">
        <div class="section-title text-center mx-auto wow fadeInUp" data-wow-delay="0.1s" style="max-width: 500px;">
            <h1 class="display-6">Modifier la publication</h1>
        </div>

        <div class="card mb-4">
            <div class="card-header bg-primary text-white">
                <h5>Modifier : {{ $publication->titre }}</h5>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('publications.update', $publication->id) }}" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <select name="categorie" class="form-control" required>
                            <option value="reemploi" {{ old('categorie', $publication->categorie) == 'reemploi' ? 'selected' : '' }}>Réemploi</option>
                            <option value="reparation" {{ old('categorie', $publication->categorie) == 'reparation' ? 'selected' : '' }}>Réparation</option>
                    ...
                    <div class="form-group">
                        ...
                        @if($publication->image)
                            ...
                            <p class="text-muted">Laissez vide pour conserver l'image actuelle.</p>
                        @endif
                    </div>
                    <button type="submit" class="btn btn-primary mt-2">Mettre à jour</button>
                    <a href="{{ route('publications.my') }}" class="btn btn-secondary mt-2">Annuler</a>
                </form>
            </div>
        </div>
    </div>
